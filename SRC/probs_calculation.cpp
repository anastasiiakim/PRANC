...
void pushNodes (int & lbl, std::stack <Node *> & stk, std::string str)
{
    ...
    while(i < (int) str.size())
    {
        ...
        // last term added to avoid treating scientific notation (ex. e-05) as label
...
void getRanks(Node* newnode, int & tail, Node ** &ar)
{
    ...
    while(newnode -> time != 0)
    {
        ...
        if(tempnode -> rank == 0) tempnode -> rank = k; //added if condition to avoid a segfault
        ++k;
        ++k1;
...
Node * mrcaST (Node * p, Node * rch, Node * lch)
{
    ...
    {
        Node * pright = mrcaST(p -> right, rch, lch);
        Node * pleft = mrcaST(p -> left, rch, lch);
...
Node * mrcaST2 (Node * p, Node * tp, std::string v)
{
    ...
    {
        Node * pleft = mrcaST2(p -> left, tp, v);	
        Node * pright = mrcaST2(p -> right, tp, v);
...
double conditionalProbability(int i, int * m_i, int *** array_k, double * s) // min i = 2 since l[i-2] exists
{
    ...
    for(int j = 0; j < m + 1; ++j)
    {
        ...
        res += (double) exp(-lambda_ij(i, j, array_k)*(s[i-2] - s[i-1]))/prod;
    }
...
{   
    ...
    calcRankedProb(arg_counter, argv, N, newnode, s, ar_y);
